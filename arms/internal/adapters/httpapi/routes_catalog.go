...
func routeCatalog() []RouteEntry {
	return []RouteEntry{
		...
		{"POST", "/api/gateway-endpoints/{id}/test-connection", "Run Mission Control connection checklist (optional JSON body { draft?: { gateway_url?, gateway_token? (omit=stored; empty=test unauthenticated), driver?, device_id?, timeout_sec? } }; returns { steps: [...] }; OpenClaw-class ws: handshake then agents.list when handshake passes; persists connection_status / pairing_* when applicable)"},
		{"DELETE", "/api/gateway-endpoints/{id}", "Remove gateway profile (409 if any execution agent references it)"},
		{"GET", "/api/research-hubs", "List ResearchClaw HTTP hubs (api_key always empty; has_api_key when stored)"},
		{"POST", "/api/research-hubs", "Create hub {base_url, optional display_name, api_key}; OpenAPI-compatible root (e.g. /api/health, /api/pipeline/start)"},
		{"PATCH", "/api/research-hubs/{id}", "Partial update (omit api_key to leave unchanged; send \"\" to clear)"},
		{"DELETE", "/api/research-hubs/{id}", "Remove hub; clears default_research_hub_id when it matches"},
		{"POST", "/api/research-hubs/{id}/test", "GET /api/health (+ /api/version when present); optional { draft?: { base_url?, api_key? } }"},
		{"POST", "/api/research-hubs/{id}/invoke", "Allowlisted GET/POST to hub (ResearchClaw OpenAPI paths); body { method, path, json_body? }; uses stored api_key"},
		{"GET", "/api/research-system-settings", "auto_research_claw_enabled + default_research_hub_id (autopilot research routing)"},
		{"PATCH", "/api/research-system-settings", "Partial update; when default_research_hub_id is non-empty it must exist"},
