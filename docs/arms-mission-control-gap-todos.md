...
**Backlog checklist (§1–§10):** `100` done · `7` open · **~93%** complete — _grep_ `- [x]` / `- [ ]` in sections below to refresh. **Master table (#1–#107):** `101` Done · `6` Open (**~94%** of numbered rows) — see **[Master backlog (all checklist items)](#master-backlog-all-checklist-items)**.

**Next priority:** **Knowledge** — **auto-ingest** from **product_feedback** / swipes / agent completion summaries (**CRUD + dispatch injection + optional `chromem` backend done**). **Convoy:** **#70**/**#71** as before. **Resilience:** **#107** auto-reassign + **#105** **live OpenClaw contracts** (baseline **shipped**: **`ARMS_OPENCLAW_LIVE_CONTRACT`** + **`internal/integration/openclaw_live_contract_test.go`**; manual workflow **`.github/workflows/openclaw-live-contract.yml`**). **Extend later:** full **stall → reassign** E2E through HTTP+worker, heartbeat RPCs if gateway exposes them. **Agents:** **#93**/**#94** still deferred. **Preference:** **embeddings** on **`preference_models`** still **TBD**.

**Asynq scheduling (steady state):** Set **`ARMS_REDIS_ADDR`** and run **`cmd/arms-worker`** alongside **`cmd/arms`**. **`ARMS_AUTOPILOT_TICK_SEC`** and **`ARMS_USE_ASYNQ_SCHEDULER`** are **deprecated** (ignored; warnings if set). **`cmd/arms`** runs startup + **5m** resync (**`product_schedules`** + per-product reconcile) plus HTTP hooks; worker runs **`product:schedule:tick`**, **`arms:product_autopilot_tick`**, optional **`arms:autopilot_tick`**, and (when **`ARMS_AUTO_STALL_NUDGE_ENABLED`**) **`arms:stall_autonudge_tick`** (same tick can **auto-reassign** when **`ARMS_AUTO_STALL_REASSIGN_ENABLED`**).
...
_Re-checked against the `arms/` tree (2026-03-24): SQLite schema **v27** (`ExpectedSchemaVersion` in `internal/adapters/sqlite/migrate.go`); baseline vs “full MC” is called out so unchecked rows are not misread as “missing entirely” when a slim table or route already exists._
...
**Still not done** (Phase A bullets): ~~**auto-reassign** for stalls (**#107**)~~ **done** (opt-in env + **`027`**); **DB leases** for task completion / product gates beyond merge-queue lease. ~~**same-Tx outbox for PR opened**~~ — after GitHub returns, task row + **`pull_request_opened`** commit together when **`LiveTX`** is wired (forge HTTP remains out-of-band).
...
| **Stall recovery → reassign** | **Auto-nudge** (**#83**) + optional **auto-reassign** (**#107** **done**): **`tasks.current_execution_agent_id`**, **`ListByProduct`**, least-loaded alternate + **`DispatchTask`** on stall; **`ARMS_AUTO_STALL_REASSIGN_*`**; SSE **`task_execution_reassigned`**. **v1 skips** **`convoy_active`**. **#105** **live OpenClaw contracts** **done** (env-gated **`TestLiveOpenClaw_*`**, workflow_dispatch CI). **Defer** MC-scale **#93**/**#94**. |
...
|-------|----------------|--------------|
| **A — Production safety** | 1–2 wk | **Done (when `AgentHealth` wired):** MC convoy singular aliases (`/api/convoy/...`); **`GET /api/products/{id}/stalled-tasks`**; completion webhook + **`POST /api/tasks/{id}/complete`** → **`task_agent_health`** **`completed`** + **`task_completed`** outbox in **one SQLite transaction** (`LiveActivityTX.CompleteTaskWithEvent`); task **`sandbox_path` / `worktree_path`** (008–009). **Manual stall nudge:** **`POST /api/tasks/{id}/stall-nudge`** (optional JSON `{ "note" }`) → `status_reason` prefix + optional agent-health `stall_nudges[]` + SSE **`task_stall_nudged`**. **Auto-nudge:** **`ARMS_AUTO_STALL_NUDGE_*`** + **`arms:stall_autonudge_tick`** (**#83**). **Auto-reassign:** **`ARMS_AUTO_STALL_REASSIGN_*`** + **#107** (same tick). **Merge queue ship:** FIFO head + **lease** + optional **real merge** (`ARMS_MERGE_BACKEND=github|local`), conflict/failure persisted on row; **`merge_ship_completed`** SSE; **autopilot merge policy** (tier + **`merge_policy_json`** gates, **semi_auto** gated auto-ship, **resolve** routes, **same-Tx outbox** on merge finish when SQLite outbox is wired) — see **Remarks — merge-queue autopilot policy** above. **Still open:** multi-instance **DB leases** for task completion / product gates beyond merge queue. |
| **B — Full autonomy** | ~2 wk | Convoy: **done (baseline DAG semantics):** `convoy_subtasks.completed` (migration 011); dependents **`dispatch-ready`** only after upstream **completed**; webhook **`convoy_id` + `subtask_id`** + parent **`task_id`**; SSE **`convoy_subtask_dispatched`** / **`convoy_subtask_completed`**. **Baseline:** parent-task **agent-health gate** + **`dispatch_attempts`** / gateway retry cap (migration **018**). **Partial (2026-03):** migration **`022`** — convoy **`metadata_json`**, subtask **`title`** / **`metadata_json`** / **`dag_layer`**, **`convoy_mail`** **`kind`** / **`from_subtask_id`** / **`to_subtask_id`**; HTTP graph summary + subtask **`status`**, enriched mail POST. **Graph slice (2026-03):** **`github.com/dominikbraun/graph`** + **`GET /api/convoys/{id}/graph`** (+ **`/api/convoy/.../graph`**) — stable **`topological_order`**, **`edges`**, **`layers`**. **DAG persistence (#66):** migration **`026` `convoy_edges`** (FK to subtasks), maintained in **`ConvoyStore.Save`**, backfilled from **`depends_on_json`**; **`GET …/convoys/{id}`** includes top-level **`edges`**. **MC task convoy HTTP (#70):** **`/api/tasks/{id}/convoy`** (+ **progress** / **dispatch** / **subtasks**), **`mc_compat`**, **`/api/convoy/{id}/mail`**. **#71** — **closed:** no separate **`tasks`** row per subtask unless a future MC integration proves it necessary (**design note** in master row **#71**). **Still TBD:** cross-agent mailbox unification. **GitHub PR + post-execution (#60)** — **done (baseline):** REST + **`gh`**, duplicate recovery, agent + **CI** HMAC webhooks, auto PR/merge path for tiers. Deeper **ideas** scoring/metadata; **`swipe_history`** table + list API (**done**); **`preference_models`** — **heuristic learning loop** (**aggregate** swipe + **`product_feedback`** + maybe-pool → **`preference_models`**, ideation bias, event-driven recompute + **`POST …/recompute`**) **done**; **ML / embeddings** still **TBD**. |
...
**Next vertical slices (suggested):** **(1)** ~~Convoy **`dominikbraun/graph`** + **`convoy_edges` (#66)** + **MC task convoy routes (#70)**~~ **done**; ~~**#71** child-task DB shape~~ **deferred / closed by design** (see master **#71**). **(2)** **Knowledge auto-ingest** (feedback, swipes, completion summaries); **semantic** backend: **`ARMS_KNOWLEDGE_BACKEND=chromem`** (**`internal/adapters/chromemstore`**) shipped; **Mem0** HTTP adapter optional later. **(3)** ~~**Stalled-task auto-reassign** (**#107**)~~ **done** — optional **convoy** subtask reassign follow-up. **(4)** ~~**Live OpenClaw contract tests** (**#105**)~~ **baseline done** — extend with E2E **stall → reassign** / heartbeat RPCs when needed. **(5)** **ML / embeddings** on **`preference_models`**. **(6)** Optional **`/api/openclaw/*`** HTTP proxy, **operations_log** breadth, PR idempotency keys (#60 polish).
...
Flat index of numbered rows **#1–#107** (§1–§10 plus cross-cutting extras). **Workflow:** update `- [ ]` / `- [x]` in the numbered sections first, then set **Status** here to **Open** or **Done** for the matching `#` row so the table stays accurate.
...
| 30 | 2 | Done | Convoy **graph algorithms** — **`github.com/dominikbraun/graph`** (stable topological sort); **`GET /api/convoys/{id}/graph`** + **`GET /api/convoy/{id}/graph`**; payload **`topological_order`**, **`edges`**, **`layers`**, **`graph`** summary; OpenAPI **`ConvoyGraphResponse`**; **`internal/application/convoy/graph.go`**. Task-scoped MC convoy routes: **#70** (§6). |
...
| 66 | 6 | Done | Persist **full** convoy DAG (MC-style analytics surface): migration **`026` `convoy_edges`** (composite FK to **`convoy_subtasks`**), rows synced in **`ConvoyStore.Save`** after subtask insert, migration **backfill** from **`depends_on_json`**; canonical graph input remains **`depends_on`** on subtasks; **`GET …/convoys/{id}`** JSON adds top-level **`edges`** (same stable order as **`GET …/graph`**). |
...
| 70 | 6 | Done | **Mission Control convoy HTTP (task-scoped)** — **`GET`/`POST`/`PATCH`/`DELETE /api/tasks/{id}/convoy`**, **`GET …/convoy/progress`**, **`POST …/convoy/dispatch`**, **`POST …/convoy/subtasks`**; **`metadata_json.mc_compat`** (`name`, `strategy`, `status`, `decomposition_spec`, `updated_at`); MC-shaped JSON (**`parent_task_id`**, **`total_subtasks`**, nested **`task`** per subtask with synthetic Kanban **`status`**, **`sort_order`**); **`strategy: ai` → 501**; **`domain.ErrConvoyExists`**; **`ConvoyRepository.ByParentTask`** + **`Delete`**; **`GET`/`POST /api/convoy/{id}/mail`** aliases; **`DispatchReady` → (dispatched int, err)**. **Not** MC’s internal **child `tasks` rows** — **#71** closed without that DB shape (see row **#71**). |
| 71 | 6 | Done | **Richer subtask / MC DB parity (#71) — closed by design:** Mission Control stores each convoy subtask as a full **`tasks`** row; ARMS keeps **`convoy_subtasks`** as the source of truth and synthesizes nested **`task`** in JSON (**#70**). **Functional parity** for closed-loop convoys (parent **agent-health** gate, **`dispatch_attempts`** + retry cap, stall **manual + auto** nudge, per-task chat, **`metadata_json`**, SSE, webhooks) is **in-tree** without duplicating Kanban rows per subtask. **Decision:** do **not** migrate to MC’s child-task schema unless a **concrete** client or operational requirement appears; if new per-subtask fields are needed, prefer columns or JSON on **`convoy_subtasks`** over a second lifecycle on **`tasks`**. |
...
| 83 | 7 | Done | **Auto**-nudge for stalled tasks — **`ARMS_AUTO_STALL_NUDGE_*`**; Asynq **`arms:stall_autonudge_tick`**; **`task_stall_nudged`** **`source":"auto"`**; auto path preserves **`last_heartbeat_at`**. Optional **auto-reassign** (**#107**) in same sweep when **`ARMS_AUTO_STALL_REASSIGN_*`** set. |
...
| 87 | 8 | Done | SSE activity (partial) — **`task_dispatched`**, **`cost_recorded`**, **`checkpoint_saved`**, **`task_completed`**, **`task_stall_nudged`**, **`task_execution_reassigned`** (#107 auto-reassign), **`pull_request_opened`**, **`merge_ship_completed`**, **`convoy_subtask_dispatched`**, **`convoy_subtask_completed`**; **`?product_id=`** filter; broader catalog + agent/type filters still TBD |
...
| 93 | 9 | Open | **Deferred (design):** MC-style **registration / discovery / import** and **gateway-backed auto-provision** — **do not** block **#107**. Revisit only if multi-tenant or large agent pools need it; until then prefer explicit **`execution_agents`** registry + minimal binding on **`tasks`**. |
| 94 | 9 | Open | **Deferred (design):** **Full MC parity** on agent **config**, **gateway import**, **aggregate health across tasks** — same rule as **#93**; ship **#107** first. Narrow future work to measured gaps (e.g. one **capability** field) rather than copying MC’s entire Agents domain. |
...
| 102 | 10 | Done | Automated tests touching persistence + HTTP wiring — SQLite `repos_test` / `migrate_test`, `sqlite_app_test`, `platform/router_test`, application tests with memory/SQLite + gateway stub; **`openclaw`** unit tests use **`httptest`** WS; **#105** live contracts in **`internal/integration/openclaw_live_contract_test.go`** (opt-in **`ARMS_OPENCLAW_LIVE_CONTRACT`**) |
...
| 104 | 10 | Done | CI — `.github/workflows/arms.yml` runs `go test ./...` and `-tags=integration` on `arms/**` changes; optional **`.github/workflows/openclaw-live-contract.yml`** (**workflow_dispatch**) runs **`TestLiveOpenClaw_*`** with repo **secrets** |
| 105 | 10 | Done | **Live OpenClaw gateway contracts (#105)** — **`internal/integration/openclaw_live_contract_test.go`**: **`TestLiveOpenClaw_ConnectAndDispatchTask`**, **`…DispatchSubtask`**, **`…BadTokenFails`**; enable with **`ARMS_OPENCLAW_LIVE_CONTRACT=1`** + **`OPENCLAW_GATEWAY_URL`** + **`ARMS_OPENCLAW_SESSION_KEY`** (+ optional token / timeout / device). Default **skip** in CI; extend later for full **stall → reassign** HTTP E2E or heartbeat RPCs if exposed. |
...
| 107 | 7 | Done | **Stalled-task auto-reassign (minimal)** — migration **`027`** **`tasks.current_execution_agent_id`**; first **`Dispatch`** binds least-loaded agent when registry wired; **`ARMS_AUTO_STALL_REASSIGN_ENABLED`** + cooldown / max-per-day; **`tryAutoReassignIfDue`** inside **`AutoNudgeStallIfDue`** before nudge; alternate via **`ListByProduct`** + **`CountByExecutionAgent`**; **`task_execution_reassigned`** event; agent-health **`stall_reassigns[]`**. **Not in v1:** **`convoy_active`** parent / subtask re-dispatch, MC **#93**/**#94**. Code: **`internal/application/task/reassign.go`**, **`reassign_test.go`**. |
...
- [x] `convoys` / `convoy_subtasks` / **`convoy_mail`** / **`convoy_edges`** — migrations **`022`** + **`026`** + HTTP: convoy **`metadata_json`**, subtask **`title`** / **`metadata_json`** / **`dag_layer`** (computed on create), mail **`kind`** / from / to; **`GET`/`POST …/convoys/{id}/mail**; **`GET …/convoys/{id}/graph`** (**`dominikbraun/graph`**: topo order, edges, layers); normalized **`convoy_edges`** for SQL joins + **`edges`** on **`GET …/convoys/{id}`** — **TBD:** MC-exact DAG HTTP spec + cross-agent mailbox unification
...
- [x] Convoy DAG — **algorithms:** **`github.com/dominikbraun/graph`** + **`GET /api/convoys/{id}/graph`** (+ alias): **`topological_order`**, **`edges`**, **`layers`**, summary. **Persistence:** migration **`026` `convoy_edges`** (FK to **`convoy_subtasks`**), maintained on **`ConvoyStore.Save`**, backfilled from existing **`depends_on_json`**; **`GET …/convoys/{id}`** includes top-level **`edges`** (stable **`from`/`to`** order).
...
- [x] API parity with MC convoy (HTTP) — task-scoped **`/api/tasks/{id}/convoy`** (+ **progress** / **dispatch** / **subtasks**), **`mc_compat`** metadata, MC-shaped subtask + nested **`task`** payloads, **`/api/convoy/{id}/mail`** aliases (**#70**). **#71 (done / design lock):** no separate **`tasks`** row per subtask; **convoy_subtasks** + **`metadata_json`** + application behavior (parent health gate, **`dispatch_attempts`**, stall nudges, chat) cover closed-loop needs — see master row **#71**; revisit schema only on proven MC-client or ops need.
...
- [x] **#107** **Auto-reassign** — **`ARMS_AUTO_STALL_REASSIGN_*`** (default **off**); migration **`027`** **`current_execution_agent_id`**; same sweep as auto-nudge; least-loaded alternate agent + **`DispatchTask`**; SSE **`task_execution_reassigned`**. **#93**/**#94** still **deferred**. **Gap:** **`convoy_active`** / subtask reassign not implemented.
...
- [x] SSE activity (partial) — **`task_dispatched`**, **`cost_recorded`**, **`checkpoint_saved`**, **`task_completed`** (SQLite same-tx + relay; in-memory hub on complete), **`task_stall_nudged`** (manual **`POST .../stall-nudge`** or auto **`arms:stall_autonudge_tick`** with optional **`source":"auto"`**), **`task_execution_reassigned`** (**#107**), **`pull_request_opened`**, **`merge_ship_completed`**, **`convoy_subtask_dispatched`**, **`convoy_subtask_completed`**; **`?product_id=`** filter; broader catalog + agent/type filters still TBD
...
- [ ] **#93** — MC-style registration / discovery / import / **gateway-backed auto-provision** — **deferred** (master row **#93**); optional future if agent pool explodes
- [ ] **#94** — **Full MC parity:** agent config depth, gateway import UX, aggregate health across tasks — **deferred** (master row **#94**); prefer incremental fields over cloning MC
...
- [x] **#105** — Live OpenClaw **contract** tests — **`ARMS_OPENCLAW_LIVE_CONTRACT`** + **`openclaw_live_contract_test.go`**; manual **`openclaw-live-contract`** workflow; **extend** with richer scenarios as needed
...
|-----------------|--------------------------------------------------------------|
| SQLite + core tables | Unblocks everything else (current **v27** migrations)     |
